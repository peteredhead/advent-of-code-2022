...
const solve = () => {
  ...
  const grid = data.map((row) =>
    row.split("").map((cell) => cell.charCodeAt(0))
  );
  const visitedGrid = data.map((row) => row.split("").map(() => false));
  const startRow = findInGrid("S", grid);
  const startColumn = findInRow("S", grid[startRow]);
  const endRow = findInGrid("E", grid);
  const endColumn = findInRow("E", grid[endRow]);

  grid[startRow][startColumn] = "a".charCodeAt(0) - 1;
