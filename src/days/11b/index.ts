...
const testDivisorProduct = Object.values(monkeys).reduce(
  (accumulator, monkey) => accumulator * monkey.testDivisor,
  1
);
...
const performOperation = (operation: string, old: number) => {
  ...
  switch (operand) {
    case "+":
      return (old + value) % testDivisorProduct;
    case "-":
      return (old - value) % testDivisorProduct;
    case "*":
      return (old * value) % testDivisorProduct;
...
const processItems = (monkeyId: string) => {
  ...
  while (monkey.startingItems.length > 0) {
    ...
    const nextMonkey =
      operationWorryLevel % monkey.testDivisor === 0
        ? monkey.trueAction
        : monkey.falseAction;
    throwToMonkey(nextMonkey, operationWorryLevel);
  }
};
