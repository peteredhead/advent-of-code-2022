...
const solve = () => {
  const rightOrderIndices = new Set();
  ...
  data.forEach((pair, index) => {
    const [_left, _right] = pair.split("\n").map((val) => JSON.parse(val));
    console.log(`\n\n\n== Pair ${index + 1} ==\n`);
    const result = compare(_left, _right);
    if (result === CORRECT) {
      rightOrderIndices.add(index + 1);
    }
  });

  return [...rightOrderIndices].reduce(
